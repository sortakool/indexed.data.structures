 ...
 */
public class IndexedByteOpenHashSet extends AbstractIndexedCollection implements IndexedByteSet {
    ...
    public IndexedByteOpenHashSet(final int expected, final float loadFactor) {
        ...
        Arrays.fill(indexes2KeysIndexes, DEFAULT_KEY_INDEX);
        keysIndexes2Indexes = new int[this.capacity];
        Arrays.fill(keysIndexes2Indexes, DEFAULT_INDEX);
    ...
    public int add(byte key) {
        //the starting point.
        int keysIndex = HashCommon.murmurHash3(key ^ mask) & mask;
    ...
    protected void rehash(final int newN) {
        ...
        Arrays.fill(newIndexes2KeysIndexes, DEFAULT_KEY_INDEX);
        Arrays.fill(newKeysIndexes2Indexes, DEFAULT_INDEX);

        for(int i=0; i< indexes2KeysIndexes.length; i++) {
            int keysIndex = indexes2KeysIndexes[i];
            if(keysIndex != DEFAULT_KEY_INDEX) {
    ...
    private int getNextFreeIndex() {
        for(int i=nextIndex+1; i< indexes2KeysIndexes.length; i++) {
            if(indexes2KeysIndexes[i] == DEFAULT_KEY_INDEX) {
                int keysIndex = indexes2KeysIndexes[i];
                assert(keysIndex == DEFAULT_KEY_INDEX);
                return i;
            }
        }
    ...
    public byte removeIndex(int index) {
        ...
        if(keysIndex != DEFAULT_KEY_INDEX) { //index has a value
            assert(used[keysIndex]);
            assert(indexes2KeysIndexes[index] != DEFAULT_INDEX);
            byte key = keys[keysIndex];
            size--;
            shiftKeys(keysIndex);
            used[keysIndex] = false;
            indexes2KeysIndexes[index] = DEFAULT_KEY_INDEX;
            keysIndexes2Indexes[keysIndex] = DEFAULT_INDEX;
    ...
    protected final void shiftKeys(int keysIndex) {
        ...
        int lastKeysIndex, slot;
        for(; ;) {
            keysIndex = ( (lastKeysIndex = keysIndex) + 1) & mask;
            while(used[keysIndex]) {
                slot = HashCommon.murmurHash3(keys[keysIndex] ^ mask) & mask;
                if(lastKeysIndex <= keysIndex ? lastKeysIndex >= slot || slot > keysIndex : lastKeysIndex >= slot && slot > keysIndex) break;
                keysIndex = (keysIndex + 1) & mask;
            }
            if(!used[keysIndex]) break;

            keys[lastKeysIndex] =  keys[keysIndex];

//            //new
//            int tempIndex = keysIndexes2Indexes[keysIndex];
//            int tempKeysIndex = indexes2KeysIndexes[tempIndex];
//            int tempLastIndex = keysIndexes2Indexes[lastKeysIndex];
//            keysIndexes2Indexes[lastKeysIndex] = keysIndexes2Indexes[keysIndex];
//            indexes2KeysIndexes[tempLastIndex] = indexes2KeysIndexes[tempIndex];
        }
        assert (used[lastKeysIndex]);
        used[lastKeysIndex] = false;

        int index = keysIndexes2Indexes[lastKeysIndex];
        keys[lastKeysIndex] = DEFAULT_KEY;
        indexes2KeysIndexes[index] = DEFAULT_KEY_INDEX;
        keysIndexes2Indexes[lastKeysIndex] = DEFAULT_INDEX;
        if(nextIndex == index) {
...
//    }
    ...
    public int remove(byte key) {
        ...
        if(index != DEFAULT_INDEX) {
            int keysIndex = indexes2KeysIndexes[index];
            int retrievedIndex  = keysIndexes2Indexes[keysIndex];
            assert(retrievedIndex == index);
            assert(used[keysIndex] == true);
            assert(keys[keysIndex] == key);
            byte removedKey = removeIndex(index);
            assert(used[keysIndex] == false);
            assert(keys[keysIndex] == DEFAULT_KEY);
